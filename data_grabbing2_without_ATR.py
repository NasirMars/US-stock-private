...
investment_data = [
    ...
    ("NVNI", "2024-12-19"),
    ("NVNI", "2024-12-20"),
    ("NVNI", "2024-12-23"),from ib_insync import IB, Stock
import sqlite3
from datetime import datetime, timedelta
import pandas as pd
import pandas_market_calendars as mcal  # Import market calendar library

# Define investment dates and symbols
investment_data = [
    ("NVNI", "2024-12-19"), # Thrusday
    ("NVNI", "2024-12-20"), # Friday
    ("NVNI", "2024-12-23"), # Monday
    ("VCIG", "2024-11-27"),
    ("MIGI", "2024-11-27"),
    ("IMCC", "2024-11-27"),
    ("IFBD", "2024-11-27"),
...
def fetch_gap_and_changes(symbol, target_date):
    contract = Stock(symbol, 'SMART', 'USD')

    # Get trading schedule around the target date
    schedule = market_calendar.schedule(start_date=target_date - timedelta(days=15), end_date=target_date + timedelta(days=15))
    trading_days = schedule.index.to_list()

    if target_date not in trading_days:
        print(f"Error: {symbol} - {target_date.date()} is not a trading day.")
        return None, None, None, None, None

    # Get the last valid trading day before target_date
    yesterday = max([day for day in trading_days if day < target_date])

    # Get the next valid trading day after target_date
    next_trading_day = min([day for day in trading_days if day > target_date])
    ...
    if not bars_daily or len(bars_daily) < 8:
        print(f"Warning: Insufficient historical data for {symbol} on {target_date.date()}")
        return None, None, None, None, None
    ...
    if not yesterday_bar or not today_bar:
        print(f"Error: Missing daily bars for {symbol} on {target_date.date()}")
        return None, None, None, None, None
